...
class WorldPage:
    ...
    async def _do_regenerate(self, dialog: ui.dialog) -> None:
        ...
        try:
            ...
            notification.message = "Step 1/7: Generating story structure..."
            ...
            notification.message = "Step 2/7: Adding characters to world..."
            ...
            notification.message = "Step 3/7: Generating locations..."
            ...
            logger.info("Generating items for the world...")
            try:
                all_existing_names = [e.name for e in self.state.world_db.list_entities()]
                item_count = random.randint(
                    self.services.settings.world_gen_items_min,
                    self.services.settings.world_gen_items_max,
                )
                item_results = await run.io_bound(
                    self.services.world_quality.generate_items_with_quality,
                    self.state.project,
                ...
                logger.info(f"Generated {len(item_results)} items from LLM")
                added_items = 0
            ...
            logger.info("Generating concepts for the world...")
            try:
                all_existing_names = [e.name for e in self.state.world_db.list_entities()]
                concept_count = random.randint(
                    self.services.settings.world_gen_concepts_min,
                    self.services.settings.world_gen_concepts_max,
                )
                concept_results = await run.io_bound(
                    self.services.world_quality.generate_concepts_with_quality,
                    self.state.project,
