...
class TestExceptionHandler:
    ...
    def _get_handler(self) -> Callable[[Exception], None]:
        ...
        with (
            ...
            # Find the handle_exception callback passed to app.on_exception
            for call in mock_app.on_exception.call_args_list:
                handler: Callable[[Exception], None] = call[0][0]
                if callable(handler):
                    return handler

        raise RuntimeError("handle_exception not registered")

    def test_parent_slot_error_logs_debug(self, caplog):
    ...
    def test_has_been_deleted_error_logs_debug(self, caplog):
