...
# =====================================================================
# Ollama API helpers (reused from evaluate_judge_accuracy.py)
# =====================================================================
...
def get_model_details(model: str) -> dict[str, Any]:
    """Get model details from Ollama (size, parameter count, etc.).
    ...
    try:
        ...
        logger.warning("Failed to get details for model '%s': %s", model, e)
        return {}
...
def test_schema_compliance(
    ...
    def on_parse_error(e: Any) -> None:
        """Capture raw response text when validation fails."""
...
def evaluate_model(
    ...
    if details:
        model_result["model_info"] = {
            "parameters": details.get("details", {}).get("parameter_size", "unknown"),
            "quantization": details.get("details", {}).get("quantization_level", "unknown"),
    ...
    for schema_name in schemas:
        ...
        # Test with max_retries=0 (raw pass/fail)
        raw_passes = 0
        ...
        for trial in range(trials):
            if verbose:
                print(f"    {schema_name} raw trial {trial + 1}/{trials}")
            trial_result = test_schema_compliance(
                model, schema_name, schema_info, temperature, timeout, 0, verbose
            )
            schema_result["raw_trials"].append(trial_result)
            raw_times.append(trial_result["response_time_seconds"])
            if trial_result["passed"]:
                raw_passes += 1
            elif trial_result["error_type"]:
                error_types.append(trial_result["error_type"])

        # Test with max_retries=3 (instructor recovery)
        retry_passes = 0
        retry_times: list[float] = []

        for trial in range(trials):
            if verbose:
                print(f"    {schema_name} retry trial {trial + 1}/{trials}")
            trial_result = test_schema_compliance(
                model, schema_name, schema_info, temperature, timeout, 3, verbose
            )
            schema_result["retry_trials"].append(trial_result)
            retry_times.append(trial_result["response_time_seconds"])
            if trial_result["passed"]:
                retry_passes += 1
            elif trial_result["error_type"]:
                error_types.append(trial_result["error_type"])
