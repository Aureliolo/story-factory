...
def get_model_size(model: str) -> dict[str, Any]:
    ...
    Returns:
        Dict with size_bytes, parameter_size, quantization.
    """
    try:
        resp = httpx.post(
        ...
        # Total model size (sum of blobs) — from modelinfo if available
        model_info = data.get("model_info", {})
        total_params = model_info.get("general.parameter_count", 0)
        return {
        ...
        logger.warning("Failed to get size for model '%s': %s", model, e)
        return {
...
def unload_model(model: str) -> dict[str, Any]:
    """Explicitly unload a model via keep_alive=0.
...
def run_scenario_sequential_unload(
    ...
    for model in models:
        ...
        unload_result = unload_model(model)
...
def main() -> None:
    ...
    model_sizes = {}
    for m in models:
        model_sizes[m] = get_model_size(m)
