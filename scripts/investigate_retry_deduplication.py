...
For model/schema pairs that FAIL (from Script 1 output), triggers structured output
failures and compares retry responses pairwise. This investigates issue #267 (C2):
no learning from repeated failures.

Hooks instructor's parse:error to capture each raw LLM response before retry.
Compares: byte-identical check, Jaccard token similarity, error escalation.
Tests temperature escalation (0.1 -> 0.3 -> 0.5 -> 0.7) across retries.
...
def load_failing_pairs(report_path: str) -> list[tuple[str, str]]:
    ...
    for model_result in report.get("model_results", []):
        ...
        for schema_name, schema_result in model_result.get("schema_results", {}).items():
            raw_pass_rate = schema_result.get("raw_pass_rate", 1.0)
