"""Cost tracking functions for ModeDatabase.

Handles generation run tracking, cost summaries, and model/entity type cost breakdowns.
"""
...
def complete_generation_run(db, run_id: str) -> None:
    """Mark a generation run as completed by setting its completed_at timestamp.
...
def get_generation_run(db, run_id: str) -> dict[str, Any] | None:
    """Retrieve a generation run record by run_id.

    Returns:
        dict: Run record with JSON fields deserialized, or None if not found.
    """
...
def get_generation_runs(
    ...
    Returns:
        list[dict[str, Any]]: A list of run records with parsed JSON fields.
    """
    logger.debug(
...
def get_cost_summary(
    ...
    """Summarize generation-run cost and usage metrics for a recent time window.
    ...
    Returns:
        dict: Summary containing total_runs, total_tokens, total_time_seconds,
        total_calls, total_iterations, wasted_iterations, avg_tokens_per_run,
        avg_time_per_run, by_run_type, and efficiency_ratio.
...
def get_model_cost_breakdown(
    ...
    Parameters:
        db: ModeDatabase instance.
        project_id (str | None): Optional project identifier to filter results.
        days (int): Lookback window in days; must be >= 0.

    Returns:
        list[dict[str, Any]]: A list of mappings per model with keys:
            model_id, call_count, total_tokens, total_time_seconds,
            avg_tokens_per_second, avg_quality.
...
def get_entity_type_cost_breakdown(
    ...
    Parameters:
        db: ModeDatabase instance.
        project_id (str | None): Optional project identifier to filter results.
        days (int): Number of days to include in the window; must be >= 0.

    Returns:
        list[dict[str, Any]]: Each dictionary contains:
            entity_type, count, total_time_seconds, avg_iterations,
            wasted_iterations, avg_quality.
        ...
        with sqlite3.connect(db.db_path) as conn:
            cursor = conn.execute(
                ...
                SELECT
                    ...
                    SUM(CASE WHEN NOT threshold_met THEN iterations_used ELSE 0 END)
                        as wasted_iterations,
