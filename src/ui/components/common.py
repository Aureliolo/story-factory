...
class LoadingSpinner:
    ...
    def build(self, inline: bool = False) -> None:
        ...
        with self._container:
            ...
            ui.label(self.message).classes("text-sm text-gray-400")
...
def empty_state(
    ...
    """
    from src.ui.theme import get_text_class
...
def loading_skeleton(width: str = "100%", height: str = "20px") -> None:
    ...
    ui.html(
        f'<div class="animate-pulse bg-gray-700 rounded" '
...
def section_header(
    ...
    with ui.row().classes("w-full items-center mb-4"):
        if icon:
            ui.icon(icon, size="md").classes("text-gray-300")
...
def progress_bar(value: float, max_value: float = 100.0, show_text: bool = True) -> None:
    ...
    with ui.column().classes("w-full gap-1"):
        if show_text:
            ui.label(f"{percentage:.0f}%").classes("text-sm text-gray-400")
