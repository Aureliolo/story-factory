"""Import wizard functions for the World page."""
...
def show_import_wizard(page) -> None:
    ...
    with ui.dialog() as dialog, ui.card().classes("p-6 min-w-[700px] max-w-[900px]"):
        ...
        with ui.row().classes("w-full justify-end gap-2 mt-4"):
            ...
            ui.button(
                "Extract Entities",
                on_click=lambda: do_import(page, dialog, text_input.value),
                icon="psychology",
            ).props("color=primary")
...
async def do_import(page, dialog: ui.dialog, text: str) -> None:
    ...
    try:
        ...
        result = await run.io_bound(
            page.services.import_svc.extract_all,
            text,
            page.state.project,
        )
        ...
        if "summary" not in result:
            ...
            try:
                ...
                logger.info(
                    f"Extraction complete: {total_entities} entities, {relationships} relationships"
                )
        ...
        await show_import_review_dialog(page, result)
                        ...
                        with ui.card().classes("w-full p-3 mb-2"):
                            with ui.row().classes("w-full items-start gap-2"):
                                ui.checkbox(
                                    ...
                                    on_change=lambda e, idx=i: selected_locs.update({idx: e.value}),
                                )
                                with ui.column().classes("flex-grow gap-1"):
                        ...
                        with ui.card().classes("w-full p-3 mb-2"):
                            with ui.row().classes("w-full items-start gap-2"):
                                ui.checkbox(
                                    ...
                                    on_change=lambda e, idx=i: selected_rels.update({idx: e.value}),
                                )
                                with ui.column().classes("flex-grow gap-1"):
                                    rel_label = ui.label(
                                    ...
                                    ui.label(f"Confidence: {rel.get('confidence', 0):.0%}").classes(
                                        "text-xs text-gray-500"
                                    )
...
async def add_imported_entities(
    page,
    ...
    try:
        ...
        for i, char in enumerate(characters):
            if selected_chars.get(i, False):
                ...
                existing = page.state.world_db.search_entities(name, entity_type="character")
                if existing:
        ...
        for i, loc in enumerate(locations):
            if selected_locs.get(i, False):
                ...
                existing = page.state.world_db.search_entities(name, entity_type="location")
                if existing:
        ...
        for i, item in enumerate(items):
            if selected_items.get(i, False):
                ...
                existing = page.state.world_db.search_entities(name, entity_type="item")
                if existing:
        ...
        for i, rel in enumerate(relationships):
            if selected_rels.get(i, False):
                ...
                if not source_id:
                    existing = page.state.world_db.search_entities(source_name, entity_type=None)
                    if existing:
                        source_id = existing[0].id

                target_id = entity_name_to_id.get(target_name)
                if not target_id:
                    existing = page.state.world_db.search_entities(target_name, entity_type=None)
                    if existing:
