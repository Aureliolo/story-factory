...
def to_epub(
    ...
    Uses the specified template (defaults to the built-in "ebook" template when None)
    and the provided options (defaults to the template's options when None). Includes
    brief metadata (title, language, description, subject) when available; converts each
    chapter with content into an individual XHTML item with escaped HTML and a bundled
    stylesheet.
    ...
    for ch in state.chapters:
        if ch.content:
            ...
            html_paragraphs = [f"<p>{html.escape(para)}</p>" for para in paragraphs if para.strip()]
