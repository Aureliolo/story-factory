"""Entity CRUD and search functions for WorldService."""
...
def delete_entity(svc: WorldService, world_db: WorldDatabase, entity_id: str) -> bool:
...
def get_entity(svc: WorldService, world_db: WorldDatabase, entity_id: str) -> Entity | None:
    """Get an entity by ID.
...
def get_entity_versions(
    svc: WorldService, world_db: WorldDatabase, entity_id: str, limit: int | None = None
) -> list:
...
def revert_entity_to_version(
    svc: WorldService, world_db: WorldDatabase, entity_id: str, version_number: int
) -> bool:
...
def find_entity_by_name(
    ...
    for entity in all_entities:
        ...
        score = calculate_name_similarity(normalized_name, entity_name)
    ...
    if best_match:
        logger.debug(
            f"Found fuzzy match for '{normalized_name}': {best_match.name} (score={best_score:.2f})"
        )
    else:
        logger.debug(
