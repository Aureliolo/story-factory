...
def validate(settings: Settings) -> None:
    ...
    _validate_data_integrity_settings(settings)
    _validate_timeout_settings(settings)
    _validate_world_quality_settings(settings)
    _validate_circuit_breaker_settings(settings)
    _validate_retry_strategy_settings(settings)
    _validate_semantic_duplicate_settings(settings)
    _validate_judge_consistency_settings(settings)
    _validate_world_generation_counts(settings)
    _validate_llm_token_limits(settings)
    _validate_entity_extraction_limits(settings)
    _validate_mini_description_settings(settings)
    ...
    _validate_world_plot_generation(settings)
    _validate_rating_bounds(settings)
    _validate_model_download_threshold(settings)
    _validate_story_chapter_counts(settings)
    _validate_import_temperatures(settings)
    _validate_token_multipliers(settings)
    _validate_content_check_settings(settings)
    _validate_world_health_settings(settings)
...
def _validate_numeric_ranges(settings: Settings) -> None:
    ...
    if not 1 <= settings.chapters_between_checkpoints <= 20:
        raise ValueError(
            f"chapters_between_checkpoints must be between 1 and 20, got {settings.chapters_between_checkpoints}"
        )

    if not 0 <= settings.max_revision_iterations <= 10:
        raise ValueError(
            f"max_revision_iterations must be between 0 and 10, got {settings.max_revision_iterations}"
        )
...
def _validate_learning_settings(settings: Settings) -> None:
    ...
    if not 1 <= settings.learning_periodic_interval <= 20:
        raise ValueError(
            f"learning_periodic_interval must be between 1 and 20, got {settings.learning_periodic_interval}"
        )

    if not 0.0 <= settings.learning_confidence_threshold <= 1.0:
        raise ValueError(
            f"learning_confidence_threshold must be between 0.0 and 1.0, got {settings.learning_confidence_threshold}"
        )


def _validate_data_integrity_settings(settings: Settings) -> None:
...
def _validate_timeout_settings(settings: Settings) -> None:
    ...
    if not 5 <= settings.subprocess_timeout <= 60:
        raise ValueError(
            f"subprocess_timeout must be between 5 and 60 seconds, got {settings.subprocess_timeout}"
        )


def _validate_world_quality_settings(settings: Settings) -> None:
    ...
    if settings.world_quality_refinement_temp_start < settings.world_quality_refinement_temp_end:
        raise ValueError(
            ...
            f"must be >= world_quality_refinement_temp_end ({settings.world_quality_refinement_temp_end}) "
            "to preserve decay semantics"
        )


def _validate_circuit_breaker_settings(settings: Settings) -> None:
...
def _validate_retry_strategy_settings(settings: Settings) -> None:
...
def _validate_semantic_duplicate_settings(settings: Settings) -> None:
...
def _validate_judge_consistency_settings(settings: Settings) -> None:
...
def _validate_world_generation_counts(settings: Settings) -> None:
...
def _validate_entity_extraction_limits(settings: Settings) -> None:
...
def _validate_mini_description_settings(settings: Settings) -> None:
...
def _validate_ollama_client_timeouts(settings: Settings) -> None:
    ...
    ollama_timeout_settings = [
        ...
        ("ollama_capability_check_timeout", settings.ollama_capability_check_timeout, 30.0, 600.0),
    ]
...
def _validate_world_plot_generation(settings: Settings) -> None:
...
def _validate_model_download_threshold(settings: Settings) -> None:
...
def _validate_story_chapter_counts(settings: Settings) -> None:
...
def _validate_content_check_settings(settings: Settings) -> None:
...
def _validate_world_health_settings(settings: Settings) -> None:
